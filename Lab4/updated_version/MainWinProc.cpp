...
LRESULT CALLBACK MainWinProc(HWND hw,UINT msg,WPARAM wp,LPARAM lp)
{
    ...
    static HFONT hfnt;
    static LPPAINTSTRUCT lpPS;
    static HDC hdcMem;
    static HBITMAP hbmMem, hbmOld;
    static HBRUSH hbrBkGnd;
    static HFONT hfntOld;
    static RECT rc;
    ...
    {
        hfnt = (HFONT)::SendMessage(hw, WM_GETFONT, 0, 0);
        hfntOld = NULL;
    ...
    case WM_MOUSEWHEEL:
    {
        int mousewheel = GET_WHEEL_DELTA_WPARAM(wp);
        if (mousewheel > 0)
        {
            VAR = minimum(VAR + 0.02, 0.356);
        }
        else
        {
            VAR = maximum(VAR - 0.02, 0.11);
        }
    }
    return 0;
    case WM_TIMER:
            ...
            case ID_TIMER1:
                {
                    ...
                    hbrBkGnd = (HBRUSH)CreateSolidBrush(GetSysColor(COLOR_WINDOW));
                    ...
                    //
                    for(int i = 0; i < (int)dynamicFigures.size(); i++)
                    ...
                    for(int i = 0; i < (int)dynamicFigures.size(); i++)
                    {
                        for(int j = 0; j < (int)staticFigures.size(); j++)
                        {
    ...
    case WM_SIZE:
    {
        ...
        for(int i = 0; i < (int)staticFigures.size(); i++)
